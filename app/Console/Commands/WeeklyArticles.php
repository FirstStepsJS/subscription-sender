...
class WeeklyArticles extends Command
{    /**
 ...
 */
     ...
     * @var string
     */
    protected $description = 'Send weekly articles Emails';
    ...
    public function getweeklySubData($userId) {

        $jwtAuthString = '';
        $client = new Client([
            'base_uri' => 'https://api.tobedeployednode.com/articles/weekly/' . $userId,
            'headers' => [
                'authorization' => 'JWT example'
            ]
        ]);
        $response = $client->request('GET');
        $weeklySubData = json_decode($response->getBody());

        return $weeklySubData->data;
    }

    public function handle()
    {
        Log::info('starting weekly subscription emails');

        $dotmailer = new Dotmailer();
        Log::info('finding subscribed articles');
        // list of subscribed members
        $members = DB::select(select * from users where subscribed_weekly = 1);
        ...
        foreach ($members as $one) {
            ...
            $articles = $this->getweeklySubData($one->id);
            $j = 1;
            $max_articles = 4;
            foreach ($articles as $article) {
                ...
                $data['article_'.$j.'_url']         = $article->application_url . "?utm_source=Email&utm_medium=Dotmailer&utm_campaign=weekly_articles";
                $data['button_'.$j.'_text']     = "Apply";
                if ($j++ > $max_articles) break;
            }
            $data['count'] = min($j, $max_articles);
            $dotmailer->sendCampaign('weekly_subscription', $one->email_address, $data);
        }
